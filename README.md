...
After applying `supabase/schema.sql` and setting `.env.local`, Phase 1 is complete when this checklist passes locally and on Vercel:

1. Start the app with `npm run dev`.
2. Visit `/`, `/login`, and `/signup` while signed out; all should load publicly.
3. Visit `/dashboard`, `/applications`, `/analytics`, `/ai-insights`, `/cv-versions`, and `/settings/profile` while signed out; each should redirect to `/login`.
4. Sign up with a real email address and confirm the email from Supabase.
5. Confirm the email link returns through `/auth/callback` and lands on `/dashboard`.
6. Confirm `/login` and `/signup` redirect authenticated users back to `/dashboard`.
7. Use the sidebar logout button and verify protected routes redirect to `/login` again.
8. In Supabase, confirm a row exists in `auth.users` and the signup trigger created the matching `users_profile` row.
...
`SUPABASE_SERVICE_ROLE_KEY` is not required by the current Phase 1 app runtime and should not be exposed to client-side code.
